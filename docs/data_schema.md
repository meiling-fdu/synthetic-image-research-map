...
One row represents one curated bibliographic paper override. This table stores bibliographic metadata and provenance for how the paper entered the dataset. Its taxonomy columns are retained only as a compatibility snapshot for the curated subset; they are not the taxonomy source of truth and cannot define public corpus membership.
...
| `venue_track` | Conference-only track: `main`, `workshops`, `findings`, `posters`, `industry`, `demo`, `doctoral_consortium`, or explicit `other`. Blank for journal, preprint, and book venues. |
...
| `tasks` | Compatibility copy for curated-subset tooling. The authoritative value is in `paper_taxonomy.csv`. |
| `image_scopes` | Compatibility copy for curated-subset tooling. The authoritative value is in `paper_taxonomy.csv`. |
| `research_types` | Compatibility copy for curated-subset tooling. The authoritative value is in `paper_taxonomy.csv`. |
...
variants) must be empty or absent. The three paper-taxonomy dimensions remain
valid for books. `publisher` is a distinct, compatible bibliographic field and
must never be placed in `venue`.
...
`research_types` is deliberately narrow. Anti-forensics, evasion, adversarial attacks, and robustness describe research topics; challenges, competitions, shared tasks, and challenge tracks describe evaluation or publication contexts. They are not research types. Papers may select multiple substantive contributions.
...
The public filters are ordered Keyword, Task, Research Type, Publication Type, Publication Venue, Country, Institution Type, Record Version, and Publication Year. Research Type describes the paper's contribution (such as Method, Dataset, Benchmark, Survey, or Analysis Study), while Publication Type describes its bibliographic format (Conference, Journal, Preprint, or Book Chapter). Every unfiltered select displays `All`. Publication Venue depends on Publication Type and uses canonical venue identities. Record Version is independent of Publication Type: it filters only whether a known arXiv alternate version exists, and deliberately has no Preprint/Published options.
...
`data/curated/author_institution_mappings.csv` keeps the reviewable paper-level grouping used by the admin and exporter. In addition to stable `mapping_id`, paper identity, `institution_id`, optional `location_id`, canonical `institution`, and normalized `institution_authors`, imported candidates preserve `author_order`, explicit one-based `affiliation_order`, `raw_affiliation`, `openalex_institution_id`, source city/country/latitude/longitude, `provenance_source`, status, and timestamps. Active and needs-review rows for a paper use contiguous `affiliation_order` values `1..N`; the Admin exporter uses this order for the unique institution list and author superscripts. New rows append at the end, exclusions compact current rows, and replace-all uses the submitted array order. `python3 scripts/migrate_affiliation_order.py` initializes legacy rows from their existing CSV/display sequence without changing other fields. Paper-level source links supply provenance, so mapping rows do not duplicate `evidence_source`, `evidence_url`, `affiliation_note`, or `review_note`. Legacy headers containing only those extra columns are read safely and rewritten to the canonical schema on the next mapping save; `python3 scripts/migrate_mapping_schema.py` performs the same atomic migration explicitly and removes the obsolete keys from generated public mapping objects. Missing coordinates never remove a candidate. Unmatched institutions use `mapping_status=needs_review`; confirmed canonical or alias matches may use `active`.
...
`data/curated/paper_taxonomy.csv` is the canonical taxonomy registry. It has one row for each reconciled public paper identity, including public records retained from preservation/candidate layers that do not have a `papers.csv` row. The exporter completes candidate selection, preservation, curated overrides, version merging, and active exclusions first; only then does it require an exact one-to-one taxonomy join. Thus taxonomy cannot be lost when candidate snapshots change, and the five actively excluded curated papers do not enter the 582-row registry or public counts.

Identity/provenance columns are `taxonomy_id`, optional curated `paper_id`, `title`, `year`, `doi`, `arxiv_id`, and `openalex_url`. `taxonomy_id` uses the strongest stable identity available (project paper ID, DOI, arXiv ID, OpenAlex ID, then normalized title/year). Each dimension has independent `{dimension}_status`, `{dimension}_review_reason`, `{dimension}_evidence_tier`, `{dimension}_evidence_source`, and `{dimension}_evidence_excerpt` fields. `taxonomy_status` is `needs_review` when any dimension needs review; it never changes bibliographic `curation_status`/`review_status` or affiliation review state.

Labels describe scope without collapsing related categories into one field:

- `tasks` contains every substantive task among `detection`, `source_attribution`, and `localization`.
- `localization` requires an explicit localization objective and evaluation. Heatmaps, explanations, local features, auxiliary segmentation, and masks supplied only as metadata do not establish it.
- `image_scopes` contains every verified scope among `fully_generated`, `generative_editing`, `deepfake`, and `traditional_manipulation`. The first two are core; the latter two retain historical and mixed-paper coverage.
- `generative_editing` requires evidence that a real/source image is modified using a generative model; generic wording about editing or manipulation is insufficient.
- `research_types` contains every substantive contribution among `method`, `dataset`, `benchmark`, `survey`, and `analysis_study`.
- Any dimension that cannot be established remains empty and sets only that dimension and the aggregate taxonomy status to `needs_review`; do not guess. An empty task array may be `reviewed` when the evidence affirmatively establishes that none of the three controlled tasks applies.
