...
Both public-preview JSON outputs include `metadata.public_preview_generated_at`, an ISO 8601 UTC timestamp written only after the refresh output passes public-preview validation. The public header formats this successful-generation timestamp as an English calendar date. Older preview files without the field remain valid and simply omit the displayed update date.
