...
When all generated institution markers for a paper are wrong, corrections belong in `data/manual/institution_record_overrides.csv`. Rows match by exact normalized title plus publication year or optional DOI/OpenAlex identifiers and currently use `mode=replace`. After finding a seed match, the exporter expands the target to every record sharing its DOI or OpenAlex URL, removes that complete record set, and inserts the manual rows at the same position in the record sequence. Each replacement supplies institution name, institution authors, location, and verified coordinates while retaining the paper's bibliographic metadata, ordered full author list, task and entry-type labels, paper/OpenAlex/arXiv provenance, confidence, and review fields. This layer corrects paper-level affiliation extraction and geocoding errors without editing raw OpenAlex data, processed affiliations, or caches, and it never calls an external API.

Both exporters run institution-record replacement before the narrower `institution_author_overrides.csv` layer, so an exact author-only correction can still refine a replacement institution later. Replacement also runs before public-preview filtering, making preview export robust when its candidate-map JSON predates a manual correction. Both summaries report manual rows loaded, papers marked for replacement, automatic records removed, replacement records created, and unmatched rows.
