...
Entry type does not replace or alter `preliminary_task`, and no candidate is excluded because of its entry type. Anti-forensics, evasion, adversarial attacks, anti-detection, counter-forensics, and robustness are topics, while challenges, competitions, shared tasks, and challenge tracks are contexts; neither group supplies entry-type values. Such papers normally remain `method` unless the title clearly identifies a dataset, benchmark, survey, or analysis contribution. Future `topic_tags` or `contribution_tags` may represent these secondary details, but they are not part of `entry_type` now. The full and in-scope candidate paper CSVs include `entry_type`; the map and public-preview exporters preserve it while accepting legacy `material_type` inputs during regeneration.
