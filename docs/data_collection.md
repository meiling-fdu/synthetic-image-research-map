...
The public-preview exporter also reads `data/manual/paper_version_overrides.csv` and the partial `data/manual/paper_arxiv_links.csv` before filtering and field limiting. This keeps known arXiv-version links available in the public preview even when the local map-ready JSON predates those manual inputs. Public-preview `arxiv_id`, `arxiv_url`, `arxiv_year`, and `has_arxiv_version` are "known arXiv version" metadata. They do not change published venue/year/DOI/OpenAlex metadata, and an arXiv version does not by itself make a formally published record preprint-only.
