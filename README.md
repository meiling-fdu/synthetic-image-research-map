...
The local admin browser provides paper search, filters, metadata inspection, associated institution-marker details, and a separate **Institution Location Review** editor. Maintainers can add papers through an OpenAlex-first search or manual fallback, curate paper-level author–institution mappings, and then confirm institution coordinates from reliable evidence. Its local refresh panel can run the fixed validation/export pipeline, display command logs, and reload preview data immediately. Confirmed records stay under `data/curated/`, and institutions missing known coordinates enter the separate location-review queue. Start it with `python3 scripts/serve_admin.py`, then open the tokenized localhost URL printed in the terminal. See the [local admin workflow](docs/admin_workflow.md) for the maintainer runbook. The admin tools never commit or push and do not add admin behavior to GitHub Pages.
