...
The local admin browser provides paper search, filters, metadata inspection, and associated institution-marker details. Maintainers can add papers through an OpenAlex-first search or manual fallback, then curate paper-level author–institution mappings without entering coordinates. Its local refresh panel can run the fixed validation/export pipeline, display command logs, and reload preview data immediately. Confirmed records stay under `data/curated/`, and institutions missing known coordinates enter the separate location-review queue. Start it with `python3 scripts/serve_admin.py`, then open the tokenized localhost URL printed in the terminal. It never commits or pushes and does not add admin behavior to GitHub Pages.
