...
The local admin browser is an **Interactive Curation Console** with a dashboard, Author Mapping Coverage, paper metadata overrides, scope review, author–institution mappings, institution location confirmation, high-risk marker review, marker blocker review, key-paper coverage review, manual/OpenAlex import review, fixed refresh workflows, validation status, Git status, and an explicit confirmed publishing action. The coverage card and searchable read-only report use `data/manual/missing_author_mappings_report.csv`, with the full generated narrative in `docs/missing_author_mappings_report.md`. Start the console with `python3 scripts/serve_admin.py`, then open the tokenized localhost URL printed in the terminal. See the [local admin workflow](docs/admin_workflow.md) for the complete maintainer runbook. Ordinary admin saves remain local; only **Publish Changes** refreshes, validates, commits selected maintained files, and pushes the current branch.

Institution review uses explicit `confirmed`, `pending_review`, `needs_coordinates`, `ambiguous`, `alias_candidate`, `alias_of_confirmed`, `ignore`, and `excluded` states. Only confirmed institutions with verified coordinates and confirmed aliases resolving through them are eligible for public map export. Multilingual and historical names are preserved in `data/curated/institution_aliases.csv` and as raw affiliation evidence; they never create independent public map nodes.

Durable maintainer decisions live in `data/curated/*.csv`. Files in `data/manual/*.csv` are generated review inputs, `web/data/*.json` is generated public-preview output, and `data/processed/*.csv` remains an unedited source layer. The intended cycle is: admin edit → **Publish Changes** → full refresh → public-preview validation → selected-file commit and push → GitHub Pages deployment.
...
Author–institution mapping gaps are listed by review priority in
`docs/missing_author_mappings_report.md`; the machine-readable version is
`data/manual/missing_author_mappings_report.csv`. Both are regenerated by the
public-preview refresh command.
