...
class PaperTaxonomyMigrationTests(unittest.TestCase):
    ...
    def test_registry_covers_the_reconciled_public_corpus_not_papers_csv(self):
        self.assertEqual(461, len(self.curated_papers))
        self.assertEqual(648, len(self.registry))
        self.assertEqual(648, len(self.public))
        self.assertEqual(450, sum(bool(row["paper_id"]) for row in self.registry))
        ...
        self.assertEqual(648, summary["public_papers_matched"])
        # The reviewed institution cleanup adds 22 located relationships.
        self.assertEqual(1486, summary["map_records_matched"])
    ...
    def test_localization_has_explicit_task_or_evaluation_evidence(self):
        ...
        self.assertEqual(29, len(localized))
    ...
    def test_generative_editing_has_source_modification_evidence(self):
        ...
        self.assertEqual(36, len(edited))
        for row in edited:
    ...
    def test_expected_counts_and_review_counts(self):
        expected = {
            "tasks": Counter(detection=608, source_attribution=79, localization=29),
            "image_scopes": Counter(fully_generated=549, generative_editing=36, deepfake=169, traditional_manipulation=14),
            "research_types": Counter(method=557, dataset=125, benchmark=77, survey=23, analysis_study=71),
        }
