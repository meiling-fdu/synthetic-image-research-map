...
Candidate extraction may retain legacy preliminary labels for review, but `data/curated/paper_taxonomy.csv` is the authoritative boundary for the three multi-label dimensions `tasks`, `image_scopes`, and `research_types`. It covers reconciled public identities rather than only `papers.csv`. Research types remain separate from formal OpenAlex `publication_type` and research topic; their allowed values are `method`, `dataset`, `benchmark`, `survey`, and `analysis_study`.

Legacy candidate labels and the curated-subset compatibility copies in `papers.csv` never override the taxonomy registry. Anti-forensics, evasion, adversarial attacks, anti-detection, counter-forensics, and robustness are topics, while challenges, competitions, shared tasks, and challenge tracks are contexts. The registry stores each taxonomy dimension as a semicolon-delimited multi-value field; public-preview JSON uses arrays and a separate `taxonomy_review` object.
...
Review the returned candidates, choose **Use this OpenAlex record**, correct or complete the prefilled metadata, assign the task, and save the confirmed record to `data/curated/papers.csv`. OpenAlex supplies candidate metadata; the saved curated row represents the maintainer's confirmation and uses `source_database=openalex`, `metadata_source=openalex`, and `curation_status=confirmed`. When an exact arXiv ID is not found in OpenAlex, the workflow may retrieve public Atom metadata directly from arXiv; a confirmed fallback row instead records `source_database=arxiv` and `metadata_source=arxiv`.

Taxonomy changes are reviewed in `data/curated/paper_taxonomy.csv`; `papers.csv` is not the taxonomy source of truth. Values are saved in canonical order; CSV uses semicolons and public JSON uses arrays. All three dimensions and their review states are independent of bibliographic `publication_type`, paper curation status, and affiliation review. Both Add Paper and Paper Metadata expose `publication_type` as a required dropdown with `conference`, `journal`, `preprint`, and `book`; legacy `article` values normalize to `journal`, while proceedings and conference-venue evidence continues to normalize to `conference`. Generated `web/data/` files are never the source of truth.
...
If no candidate is correct—or OpenAlex is unavailable—choose **Add manually instead**. The same form opens without metadata, preserves manual provenance in the source fields, and saves `curation_status=confirmed`. OpenAlex failures do not write any files and do not disable this manual fallback.
...
Curated mappings produce markers only when the author–institution relationship is explicitly source-backed, the canonical Institution Record is active/resolved, and a maintainer-confirmed record in `data/curated/institution_locations.csv` supplies valid coordinates. An active mapping remains eligible as before. A `needs_review` mapping may also be eligible while its paper remains `needs_review`, but only when `raw_affiliation` and `provenance_source` preserve the paper evidence and both the institution identity and location are independently confirmed. Pending identities, pending locations, cache-only coordinates, and ambiguous matches remain non-geographic; their affiliations can still appear as preliminary paper details. A confirmed-location marker carries `resolution_method=curated_confirmed_location`. No paper review status is changed by marker eligibility, and no coordinates are inferred from affiliation text.

Active mappings without a unique valid location are added or updated in `data/curated/institution_location_review.csv`. Missing matches use `location_status=missing` and `coordinate_status=missing`; ambiguous matches use `needs_coordinate_review` and `ambiguous`. Creation timestamps are preserved, and repeated mappings for the same paper and institution do not create duplicate queue items. When a previously queued institution gains an exact known location, its queue status is updated to `known`.
...
Open the tokenized local admin browser and choose **Institution Location Review** to inspect rows from `data/curated/institution_location_review.csv`. Each queue row keeps the related paper, institution authors, raw affiliation, mapping evidence, suggested location, and current status visible. Location review is intentionally separate from confirming the paper-level author–institution relationship.

Select a queue row and enter the confirmed institution, city/region/country labels, uppercase two-letter country code, latitude, and longitude. Latitude must be between -90 and 90 and longitude between -180 and 180. The server never guesses or invents a coordinate; the maintainer reviews any suggested match before confirmation.

Confirmation atomically creates or updates the matching normalized institution in `data/curated/institution_locations.csv`, marks the queue row `location_status=known` and `coordinate_status=known`, and preserves the queue's paper and evidence context. Duplicate confirmed rows for the same normalized institution are rejected. **Mark ambiguous** records `location_status=ambiguous` and `coordinate_status=needs_coordinate_review`; Ignore and Exclude remain durable exception states under **More actions**.
...
By default, the exporter publishes eligible `in_scope=true` records with at least one audited task, image scope, and research type; requires `resolution_confidence` of `medium` or `high`; and excludes records marked `needs_review=true`. It also requires a non-placeholder institution name and a finite latitude/longitude pair within geographic bounds. The exporter publishes the three taxonomy dimensions as arrays and never exposes retired scalar task/category fields.
