#!/usr/bin/env python3
"""Integrate only confirmed, missing papers from the Tier 2 NORMAL evidence pass."""
...
from systematic_tier2_normal_priority_evidence_data import (
    DECISIONS,
...
def paper_id(candidate_id: str) -> str:
    return hash_id(
        "curated:",
        "systematic-tier2-normal-priority-evidence:" + candidate_id,
        20,
    )
...
def institution_id(name: str) -> str:
    return hash_id(
        "institution:",
        "systematic-tier2-normal-priority-evidence:" + name,
        16,
    )
...
def identity_preflight() -> list[dict[str, object]]:
    """Reconcile all nine additions against the actual 657-paper baseline."""
    public = json.loads(
...
def validate_scope_and_queue() -> dict[str, dict[str, str]]:
    ...
    assert len(normal) == 20 and set(normal) == set(decisions)
    assert all(normal[cid]["title"] for cid in normal)
    outcomes = [row["evidence_resolution_outcome"] for row in DECISIONS]
    assert outcomes.count("EVIDENCE_CONFIRMS_SCOPE") == 9
    assert outcomes.count("EVIDENCE_EXCLUDES_SCOPE") == 11
...
def build(*, allow_integrated: bool = False) -> dict[str, object]:
    ...
    for paper in PAPERS:
        ...
        for dimension in ("tasks", "image_scopes", "research_types"):
            ...
            taxonomy[dimension + "_review_reason"] = (
                "Primary-source review under the narrowed Tier 2 NORMAL evidence policy."
            )
            taxonomy[dimension + "_evidence_tier"] = "primary_paper"
            taxonomy[dimension + "_evidence_source"] = str(decision["primary_source"])
            taxonomy[dimension + "_evidence_excerpt"] = str(
                paper.get(
                    dimension + "_evidence",
    ...
    assert len(mapping_rows) == 21
    assert len(new_institutions) == len(location_review_rows) == 5
    return {
