#!/usr/bin/env python3
"""Render and validate the Tier 2 NORMAL evidence-resolution successor layer."""
...
from integrate_systematic_tier2_normal_priority_evidence import (
    OUT,
    QUEUE,
    ROOT,
...
from systematic_tier2_normal_priority_evidence_data import DECISIONS, PAPERS


CSV_PATH = ROOT / "data/manual/systematic_tier2_normal_priority_evidence_review_2026_09.csv"
REPORT_PATH = ROOT / "docs/systematic_tier2_normal_priority_evidence_review_2026_09.md"
FIELDS = (
    "candidate_id",
    "title",
...
def validate_lock_receipts(rows: list[dict[str, str]]) -> dict[str, object]:
    evidence_lock = json.loads((OUT / "evidence_lock.json").read_text(encoding="utf-8"))
    evidence_counts = Counter(row["evidence_resolution_outcome"] for row in rows)
    assert evidence_lock == {
        "candidate_ids": [row["candidate_id"] for row in rows],
        "evidence_projection_sha256": projection_sha256(rows, EVIDENCE_LOCK_FIELDS),
        "outcomes": dict(sorted(evidence_counts.items())),
        "priority": "NORMAL",
        "queue": "queue_c_evidence_required.csv",
        "rows": 20,
    }
    ...
    identity_rows = [
        ...
        if decision["evidence_resolution_outcome"] == "EVIDENCE_CONFIRMS_SCOPE"
    ]
    identity_lock = json.loads((OUT / "identity_lock.json").read_text(encoding="utf-8"))
    identity_counts = Counter(
        str(row["identity_reconciliation_outcome"]) for row in identity_rows
    )
    assert identity_lock == {
        "candidate_ids": [str(row["candidate_id"]) for row in identity_rows],
        "identity_projection_sha256": projection_sha256(
            identity_rows, IDENTITY_LOCK_FIELDS
        ),
        "outcomes": dict(sorted(identity_counts.items())),
        "rows": 9,
    }
    return {"evidence": evidence_lock, "identity": identity_lock}
...
def expected_rows() -> list[dict[str, str]]:
    queue = {
        ...
        if row["evidence_review_priority"] == "NORMAL"
    }
        ...
        pending_coordinates = sorted(
            {
                ...
                for affiliation in affiliations
                if affiliation["institution_id"] not in known_location_ids
            }
        )
        issues: list[str] = []
        if candidate_id == "audit:29fa62c1fc9fd79d":
            issues.append("No DOI, arXiv ID, or OpenAlex ID was established.")
        if pending_coordinates:
            issues.append(
                "Coordinates pending for: " + "; ".join(pending_coordinates) + "."
            )
        if not confirmed:
            issues.append("None; scope decision is final for this pass.")
...
def diff_audit() -> dict[str, object]:
    ...
    assert sorted(frontend) == current_frontend, {
...
def render(rows: list[dict[str, str]], stats: dict[str, object], diff: dict[str, object]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Corpus impact",
            "",
            f"- Public papers: 657 → {stats['public']}",
            f"- Published-only papers: 546 → {stats['published']}",
            f"- Mapped papers: 637 → {stats['mapped']}",
            f"- Map markers: 1,494 → {stats['markers']}",
...
def main() -> None:
    ...
    validate_lock_receipts(expected)
    if args.write:
